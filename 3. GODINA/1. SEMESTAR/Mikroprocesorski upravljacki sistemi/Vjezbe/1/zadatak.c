...
void convertStr(char* op, unsigned int* opN, char type, int size) {
    ...
    if(type == 'h') {
        for(i = size; i > 0; i--) {
            if(*(op+i) >= '0' && *(op+i) <= '9') {
                *opN += hexDeg[size-i] * ((unsigned int) *(op+i) - 48);
                //*opN += pow(16, size-i) * ((unsigned int) *(op+i) - 48);
            }
            else if(*(op+i) >= 'A' && *(op+i) <= 'F') {
                *opN += hexDeg[size-i] * ((unsigned int) *(op+i) - 55);
        ...
        for(i = size; i > 0; i--) {
            if(*(op+i) != '1' && *(op+i) != '0') {
               ...
               *opN += binDeg[size-i] * ((unsigned int) *(op+i) - 48); 
        ...
        for(i = size; i > 0; i--) {
            if(*(op+i-1) >= '0' && *(op+i-1) <= '9') {
                *opN += decDeg[size-i] * ((unsigned int) *(op+i-1) - 48);
